function init(){
    var margin = {top: 20, right: 20, bottom: 30, left: 40},
    width = 960 - margin.left - margin.right,
    height = 500 - margin.top - margin.bottom;

    var x = d3.scale.ordinal()
	.rangeRoundBands([0, width], .1);

    var y = d3.scale.linear()
	.rangeRound([height, 0]);
    
    var color = d3.scale.ordinal()
	.range(["#98abc5", "#8a89a6", "#7b6888", "#6b486b", "#a05d56", "#d0743c", "#ff8c00"]);

    var xAxis = d3.svg.axis()
	.scale(x)
	.orient("bottom");
    
    var yAxis = d3.svg.axis()
	.scale(y)
	.orient("left")
	.tickFormat(d3.format(".2s"));

    var svg = d3.select("#vis").append("svg")
	.attr("width", width + margin.left + margin.right)
	.attr("height", height + margin.top + margin.bottom)
	.append("g")
	.attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    
    //var parseDate = d3.time.format("%Y-%m-%d %H:%M").parse;

    d3.json("data/uber.json", function(error, data){
	if (error) throw error;
	/*
	data.forEach(function(d) {
	    timestamp.push(parseDate(d.timestamp));
	});
	*/

	color.domain(d3.keys(data[0]).filter(function(key) {return key == "estimated_waiting_time" || key == "duration"; }));
	
	data.forEach(function(d) {
	    var y0 = 0;
	    d.span = color.domain().map(function(name) { return {name: name, y0: y0, y1: y0 += +d[name]}; })
	    d.total = d.span[d.span.length - 1].y1;
	});


	/*
	    d.time = parseDate(d.timestamp);
	    d.wait = +d.estimated_waiting_time / 60.0;
	    d.duration = +d.duration / 60.0;
	});
	
	data.sort(function(a, b) { return b.total - a.total; });
	*/
	/*
	var xscale = d3.time.scale().domain(d3.extent(timestamp, function(d) { 
	    return d; })).range([0,width]);
	
	var xAxis = d3.svg.axis()
	    .scale(xscale)
	    .orient("bottom");
	
	var y = d3.scale.linear()
	    .rangeRound([height, 0]);

	y.domain([0, d3.max(data, function(d) { return d.total; })]);

	var yAxis = d3.svg.axis()
	    .scale(y)
	    .orient("left")
	    .tickFormat(d3.format(".2s"));
	    */

	x.domain(data.map(function(d) { return d.timestamp; }));
	y.domain([0, d3.max(data, function(d) { return d.total; })]);
	
	svg.append("g")
	    .attr("class", "x axis")
	    .attr("transform", "translate(0," + height + ")")
	    .call(xAxis);

	svg.append("g")
	    .attr("class", "y axis")
	    .call(yAxis)
	    .append("text")
	    .attr("transform", "rotate(-90)")
	    .attr("y", 6)
	    .attr("dy", ".71em")
	    .style("text-anchor", "end")
	    .text("Timespan");

	var timestamp = svg.selectAll(".timestamp")
	    .data(data)
	    .enter().append("g")
	    .attr("class", "g")
	    .attr("transform", function(d) { return "translate(" + x(d.timestamp) + ",0)"; });

	timestamp.selectAll("rect")
	    .data(function(d) { return d.span; })
	    .enter().append("rect")
	    .attr("width", x.rangeBand())
	    .attr("y", function(d) { return y(d.y1); })
	    .attr("height", function(d) { return y(d.y0) - y(d.y1); })
	    .style("fill", function(d) { return color(d.name); });

	var legend = svg.selectAll(".legend")
	    .data(color.domain().slice().reverse())
	    .enter().append("g")
	    .attr("class", "legend")
	    .attr("transform", function(d, i) { return "translate(0," + i * 20 + ")"; });

	legend.append("rect")
	    .attr("x", width - 18)
	    .attr("width", 18)
	    .attr("height", 18)
	    .style("fill", color);

	legend.append("text")
	    .attr("x", width - 24)
	    .attr("y", 9)
	    .attr("dy", ".35em")
	    .style("text-anchor", "end")
	    .text(function(d) { return d; });

    })
};
